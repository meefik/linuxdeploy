...
{
    local pidfile pid
    pidfile="${CHROOT_DIR}/tmp/xsession.pid"
    [ -e "${pidfile}" ] && pid=$(cat "${pidfile}")
    if [ -n "${pid}" -a -e "/proc/${pid}" ]; then
        return 0
    else
...
container_umount()
{
    ...
    for item in '.*' '*'
    do
        local parts=$(cat /proc/mounts | awk '{print $2}' | grep "^${CHROOT_DIR}/${item}$" | sort -r)
        local part
        for part in ${parts}
        do
            local pp=$(echo ${part} | sed "s|^${CHROOT_DIR}/*|/|g")
            msg -n "${pp} ... "
...
container_status()
{
    ...
    local item
    for item in $(grep -v "${CHROOT_DIR}" /proc/mounts | grep ^/ | awk '{print $2":"$3}')
    do
        local part=$(echo ${item} | awk -F: '{print $1}')
        local fstype=$(echo ${item} | awk -F: '{print $2}')
